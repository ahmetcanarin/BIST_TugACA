...
def generate_champion_actionable_signals(
    ...
    day_df = df[df["Date"] == latest_date].sort_values(score_col, ascending=False)
    avg_expected_excess = float(day_df.head(10)[reg_col].mean())
    ...
    is_cash_day = avg_expected_excess < conviction_threshold
    if is_cash_day:
...
def run_head_to_head_benchmark(
    ...
    configs = [
        ("1. Model A: Nedensel GRU (Saf Hisse / 0% Nakit)", "rank_gru", None, 0.0, 0.0),
        ("2. Model B: LightGBM (Saf Hisse / 0% Nakit)", "rank_lgbm", None, 0.0, 0.0),
        ("3. Model A (GRU) + Kalibre Conviction (Eşik -%0.10, %35 PPF)", "rank_gru", "pred_gru_reg", -0.10, rf_daily),
        ("4. Model A (GRU) + Kalibre Conviction (Eşik -%0.25, %35 PPF)", "rank_gru", "pred_gru_reg", -0.25, rf_daily),
        ("5. Model B (LGBM) + Kalibre Conviction (Eşik -%0.10, %35 PPF)", "rank_lgbm", "pred_lgbm_reg", -0.10, rf_daily),
        ("6. Model B (LGBM) + Kalibre Conviction (Eşik -%0.25, %35 PPF)", "rank_lgbm", "pred_lgbm_reg", -0.25, rf_daily),
        ("7. Model A (GRU) + Aşırı Defansif (Eşik %0.0, %35 PPF) [V4 Nakit Kapanı]", "rank_gru", "pred_gru_reg", 0.0, rf_daily)
    ]
    ...
    for name, sig_col, conv_col, conv_thresh, cash_yield in configs:
...
if __name__ == "__main__":
    ...
    df_signals = test_lag[["Date", "Ticker", "Target_Return", "Target_Excess_Return"]].copy()
    ...
    # 6. Şampiyon Model Seçimi (Saf Hisse Alfasını En Yüksek Üreten Model)
    gru_saf = benchmark_results["1. Model A: Nedensel GRU (Saf Hisse / 0% Nakit)"]
    lgbm_saf = benchmark_results["2. Model B: LightGBM (Saf Hisse / 0% Nakit)"]
    ...
    if gru_alpha >= lgbm_alpha:
        ...
        champ_bt = benchmark_results["3. Model A (GRU) + Kalibre Conviction (Eşik -%0.10, %35 PPF)"]
    else:
        champion_name = "LightGBM"
        champ_score_col = "rank_lgbm"
        champ_reg_col = "pred_lgbm_reg"
        champ_prob_col = "pred_lgbm_prob"
        champ_bt = benchmark_results["5. Model B (LGBM) + Kalibre Conviction (Eşik -%0.10, %35 PPF)"]
